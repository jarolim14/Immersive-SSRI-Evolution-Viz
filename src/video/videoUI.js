...
class VideoUI {
  ...
  initialize(canvas, scene, camera, controls) {
    ...
    try {
      ...
      if (
        CONFIG.development.videoRecording.showButton &&
        CONFIG.development.videoRecording.showAllUI
      ) {
  ...
  createRecordButton() {
    ...
    this.recordButton.addEventListener("click", () => {
      ...
      this.recordButton.style.display = "none"; // Hide button after click
  ...
  initializeVideoRecordingModal() {
    ...
    if (!modal) {
      console.error(
        "Video recording modal element not found. Cannot initialize."
      );
      return;
    }
  ...
  showVideoRecordingModal() {
    ...
    if (this.videoRecorder) {
      ...
      console.log(
        "Skipping compatibility check: videoRecorder not initialized yet"
      );
      ...
      fallbackMessage.textContent =
        "Video recorder not initialized yet. Please try again in a moment.";
  ...
  updateCompatibilityStatus() {
    if (!this.videoRecorder) {
      console.warn(
        "Cannot update compatibility status: videoRecorder is not initialized yet"
      );
      return;
    }
    ...
    if (
      ...
      console.error(
        "One or more compatibility status elements not found in DOM"
      );
      return;
    }
    ...
    mediaRecorderStatus.textContent = hasMediaRecorder
      ? "✓ Supported"
      : "✗ Not Supported";
    mediaRecorderStatus.className =
      "status " + (hasMediaRecorder ? "supported" : "not-supported");
    ...
    webmVp9Status.className =
      "status " + (hasWebmVp9 ? "supported" : "not-supported");
    ...
    webmVp8Status.className =
      "status " + (hasWebmVp8 ? "supported" : "not-supported");
    ...
    try {
      ...
      if (this.videoRecorder) {
        await this.videoRecorder.startRecording(
          CONFIG.development.videoRecording.defaultDuration
        );
      ...
      errorNotice.textContent = `Recording error: ${
